    ...
    return(
        <>
            ...
            <h1>Training</h1>
                            ...
                            <h2>Held Items :  <span className={"pokemon-detail-value "+ color} style={{background: 'none', boxShadow:'none'}}><div className="held-items-wrapper">{props.heldItems.length!==0 ? props.heldItems.map(x=>  <div key={x.item?.name}>{firstLetterUpper(x.item.name.split('-').join(' '))}  {x.version_details[0].rarity}% </div>): "None"}</div></span>
                            </h2>
