...
const useHttp = (url)  => {
    ...
    const fetchPokemonData = async () => {
        setLoading(true)
        const data = await axios("https://pokeapi.co/api/v2/pokemon/?offset=0&limit=1154").then(response=>response.data)
    ...
    const fetchPokemonPageData= async () =>{
        ...
        if (data3.id <= 810){
            ...
            const pokeAlternativeName = data3.name.split('-').join(' ').substring(0, data3.name.split('-').join(' ').indexOf(' '))
            console.log(pokeAlternativeName)
            const data4 = await axios("https://pokeapi.co/api/v2/pokemon-species/"+pokeAlternativeName).then( (response) => response.data)
            const data5 = await axios(data4.evolution_chain.url).then((response) => response.data)
